...
namespace miniStore.AdminApp.Controllers
{
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Index(string keyword,int? categoryId,int pageIndex = 1, int pageSize =10 )
        {
            ...
            var products = await _productApiClient.GetAllPagings(request);
        ...
        public async Task<IActionResult> Edit([FromForm] ProductUpdateRequest request)
        {
            ...
            if (result)
            {
                TempData["result"] = "Update product successful";
                return RedirectToAction("Index");
            }
        ...
        public async Task<IActionResult> Delete(ProductDeleteRequest request)
        {
            ...
            if (result)
            {
                TempData["result"] = "Delete product successful";
                return RedirectToAction("Index");
            }
        ...
        private async Task<CategoryAssignRequest> GetCategoryAssignRequest(int id)
        {
            ...
            foreach (var cate in categories)
            {
                categoryAssignRequest.Categories.Add(new SelectItem
                {
                    Id=cate.Id.ToString(),
                    Name=cate.Name,
                    Selected = productObj.Categories.Contains(cate.Name)
                });
